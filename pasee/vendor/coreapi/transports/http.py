...
from pasee.vendor.coreapi import exceptions, utils
from pasee.vendor.coreapi.compat import cookiejar, urlparse
from pasee.vendor.coreapi.document import Document, Object, Link, Array, Error
from pasee.vendor.coreapi.transports.base import BaseTransport
from pasee.vendor.coreapi.utils import guess_filename, is_file, File
import collections
import requests
from pasee.vendor import itypes
import mimetypes
import uritemplate
import warnings
...
def _get_headers(url, decoders):
    ...
    headers = {"accept": ", ".join(accept_media_types), "user-agent": "coreapi"}
...
def _coerce_to_error_content(node):
    ...
    if isinstance(node, (Document, Object)):
        ...
        return OrderedDict(
            [(key, _coerce_to_error_content(value)) for key, value in node.data.items()]
        )
...
def _coerce_to_error(obj, default_title):
    ...
    if isinstance(obj, Document):
        return Error(
            title=obj.title or default_title, content=_coerce_to_error_content(obj)
        )
...
class HTTPTransport(BaseTransport):
    ...
    def __init__(
        ...
        if request_callback is not None or response_callback is not None:
            ...
            session.mount(
                "https://", CallbackAdapter(request_callback, response_callback)
            )
            session.mount(
    ...
    def transition(
        self, link, decoders, params=None, link_ancestors=None, force_codec=False
    ):
        ...
        settings = session.merge_environment_settings(
            request.url, None, None, None, None
        )
